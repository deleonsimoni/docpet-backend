                ...
                .then(async function(veterinarios) {
                    ...
                    await estabelecimentoModel.find({ nomeFormated: { $regex: '.*' + findRegex + '.*', $options: 'i' } }).select('nome nomeFormated')
                        .then(async function(estabelecimento) {
                            ...
                            retorno = [...retorno, ...estabelecimento.map(obj => ({...obj._doc, type: 3 }))];
                    ...
                    await adestradorModel.find({ nomeFormated: { $regex: '.*' + findRegex + '.*', $options: 'i' } }).select('nome nomeFormated')
                        .then(async function(adestrador) {

                            retorno = [...retorno, ...adestrador.map(obj => ({...obj._doc, type: 4 }))];
                    ...
                    await esteticaModel.find({ nomeFormated: { $regex: '.*' + findRegex + '.*', $options: 'i' } }).select('nome nomeFormated')
                        .then(async function(estetica) {

                            retorno = [...retorno, ...estetica.map(obj => ({...obj._doc, type: 5 }))];
